...
if minor > MINOR then return end
...
local function closeEnough(a, b) return math.abs(a - b) < 0.01 end
...
local function isDefaultPosition(parent)
	...
	if not default then default = CopyTable(CENTER) end
...
function dialogMixin:ResetPosition()
	...
	if not pos then pos = CopyTable(CENTER) end
